...
-- =========================================================
-- MODELO NUEVO SIN COMISIONES:
-- - alumno_materia_anio reemplaza alumno_comision
-- - evaluaciones/clases pasan a materia_id + anio
-- - se eliminan comisiones y alumno_comision
-- =========================================================
...
-- 2) migrar datos desde comisiones + alumno_comision (si existen)
...
-- 3) agregar columnas nuevas a evaluaciones/clases
...
-- poblar evaluaciones desde comisiones
...
-- poblar clases desde comisiones
...
alter table public.evaluaciones
  add constraint if not exists evaluaciones_materia_fk
  foreign key (materia_id) references public.materias(id) on delete cascade;

alter table public.clases
  add constraint if not exists clases_materia_fk
  foreign key (materia_id) references public.materias(id) on delete cascade;

-- unique por modelo nuevo
...
-- 4) notas trigger: validar contra alumno_materia_anio + evaluaciones(materia_id,anio)
...
-- 6) policies alumno_materia_anio
...
-- 7) policies evaluaciones/notas por materia_id
...
-- 8) policies clases/asistencias por materia_id
...
-- 9) limpieza final: remover modelo viejo
alter table public.evaluaciones drop column if exists comision_id;
alter table public.clases drop column if exists comision_id;

drop table if exists public.alumno_comision cascade;
